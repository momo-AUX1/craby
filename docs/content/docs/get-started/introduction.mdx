...
- <TossFace>✅</TossFace> Performance-critical native modules (e.g., large-scale data processing, cryptographic operations, real-time parsing)
- <TossFace>✅</TossFace> High-performance file I/O operations with large datasets
- <TossFace>✅</TossFace> Building high-performance modules in Rust with type safety
...
- <TossFace>❌</TossFace> Modules requiring platform-specific features (e.g., camera access, sensors, native UI components)
- <TossFace>❌</TossFace> Modules requiring standard TurboModule capabilities (e.g., event emitters, platform context access)
